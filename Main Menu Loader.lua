HackersHouse = {
    ...
    ['checkUpdate'] = function ()
        ...
        codeFromServer = gg.makeRequest('').content
        if not codeFromServer then
            codeFromServer2 = gg.makeRequest('').content
...
function field()
fco1=24 fco2=8 fco3=16 fcx1=32
E=0 gg.toast("Searching Value...") gg.clearResults() 
gg.setRanges(gg.REGION_C_DATA | gg.REGION_C_ALLOC | gg.REGION_ANONYMOUS | gg.REGION_OTHER) 
lengthf=#(f) gg.searchNumber(":"..tostring(f),1)
    if gg.getResultsCount()==0 then E=0 return end 
fbfc=gg.getResults(1) gg.refineNumber(fbfc[1].value,1)
fbfc=gg.getResults(gg.getResultsCount())
nfbx=nil nfbx={} 
    for i, v in ipairs(fbfc) do nfbx[i]={} nfbx[i].address=v.address+lengthf nfbx[i].flags=1 end
gg.loadResults(nfbx) gg.refineNumber(0,1)
    if gg.getResultsCount()==0 then E=0 return end 
fbfc=gg.getResults(gg.getResultsCount())
nfbx=nil nfbx={} 
    for i, v in ipairs(fbfc) do nfbx[i]={} nfbx[i].address=v.address-(lengthf+1) nfbx[i].flags=1 end
gg.loadResults(nfbx) gg.refineNumber(0,1)
    if gg.getResultsCount()==0 then E=0 return end 
fbfc=gg.getResults(gg.getResultsCount())
nfbx=nil nfbx={} 
    for i, v in ipairs(fbfc) do nfbx[i]={} nfbx[i].address=v.address+1 nfbx[i].flags=1 end
gg.loadResults(nfbx) 
gg.searchPointer(0) 
    if gg.getResultsCount()==0 then E=0 return end 
fpfc=gg.getResults(gg.getResultsCount())
gg.clearResults() fofc=nil fofc={}
    for i, v in ipairs(fpfc) do fofc[i]={} fofc[i].address=v.address+fco1 fofc[i].flags=4 end
gg.loadResults(fofc) 
gg.refineNumber("4~5000",4)
    if gg.getResultsCount()==0 then E=0 return end 
fofc=gg.getResults(gg.getResultsCount())
gg.clearResults() fcfc={}
    for i, v in ipairs(fofc) do fcfc[i]={} fcfc[i].address=v.address-fco2 fcfc[i].flags=fcx1 end
fcfc=gg.getValues(fcfc) hcfc={} 
    for i, v in ipairs(fcfc) do    
        hcfc[i]={} hcfc[i].flags=fcx1
        if is64 then HCFC=tostring(string.format("%X",tostring(fcfc[i].value)))   
            if #(HCFC)>=12 then HCFC=tostring(string.sub(HCFC,3,-1)) end
            HCFC=tonumber("0x"..HCFC)+fco3 
        else HCFC=(fcfc[i].value)+fco3 
        end
        hcfc[i].address=HCFC
    end 
...
cnfc={}
    for i, v in ipairs(hcfc) do
        for c=1,50 do 
            cnfc[c]={} cnfc[c].address=v.value+c-1 cnfc[c].flags=1 
        end
            ...
            if cnfc[c].value~=0 and cnfc[c].value>32 and cnfc[c].value<127 then 
                ...
                csfc=table.concat(csfc)
                    if tostring(csfc)==tostring(X) then 
                        o=tonumber(fofc[i].value) FCFC=fcfc[i].value E=1
                        break return
                    else
                        break 
                    end
    ...
    if E==1 then 
      ...
      gg.searchNumber(FCFC,fcx1)
        ...
        FCFC=nil FCFC={} 
            for i, v in ipairs(fcfc) do
                FCFC[i]={} FCFC[i].address=v.address+o FCFC[i]. flags=t
            end
  gg.loadResults(FCFC) 
    end 
end 
